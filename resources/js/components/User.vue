        ...
        <div class="p-4 flex justify-between items-center">
            ...
            <div class="flex items-center">
                <div class="relative text-gray-600 mr-4">
                    <select v-model="role" class="border border-yellow-500 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none"
                    type="search" name="search" placeholder="Name">
                        <option value="Administrator">Administrator</option>
                        <option value="Mid">Mid User</option>
            ...
            <table class="w-full text-md bg-white shadow-md rounded mb-4">
                <tbody>
                    <tr class="border-b text-gray-900">
                        <th class="text-left fali-color p-3 px-5">Name</th>
                        <th class="text-left fali-color p-3 px-5">Username</th>
                    ...
                    <tr v-show="user.id != userInfo.id" v-for="user in users.data" class="border-b hover:bg-yellow-200 bg-gray-100">
                        ...
                        <td class="p-3 px-5 flex justify-end">
                            <button @click="editAgentModal(user)" type="button" class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline flex">
                ...
                <div class="personal w-full border-t border-gray-400 pt-4 my-auto">
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <div class='w-full px-3 mb-4'>
                            ...
                            <input v-model="form.username" class='appearance-none block w-full bg-white text-gray-700 border-orange-400 border shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-orange-500' type='text'  required>
                        </div>
                    </div>
                    <div class="flex items-center justify-between mt-4">
                        <div class='w-full px-3 mb-4'>
                            <label class='block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2' >User Role</label>
                            <select v-model="form.role" class='appearance-none block w-full bg-white text-gray-700 border-orange-400 border shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-orange-500' type='text'  required>
                                <option value="Administrator">Administrator</option>
                                <option value="Mid">Mid User</option>
                                <option value="Viewonly">View Only</option>
                            </select>
                    ...
                    <div v-show="!isEdit" class="flex items-center justify-between mt-4">
                        <div class='w-full px-3 mb-4'>
                            <label class='block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2' >Password</label>
                            <input v-model="form.password" class='appearance-none block w-full bg-white text-gray-700 border-orange-400 border shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-orange-500' type='password'  required>
                        </div>
                    </div>
                    <div v-show="!isEdit" class="flex items-center justify-between mt-4">
                        <div class='w-full px-3 mb-4'>
                            <label class='block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2' >Retype Password</label>
                            <input v-model="form.retypePassword" class='appearance-none block w-full bg-white text-gray-700 border-orange-400 border shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-orange-500' type='password'  required>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async deleteUser (id) {
                this.isLoading = true
                await this.$store.dispatch('user/deleteUser', { id: id })
                this.getAgents()
                this.isLoading = false
            },
            confirmDelete (id) {
